  ...
  const generateContent = async (customInstructions?: string) => {
    ...
    try {
      ...
      const summaryParts = parsed
        ...
        .filter(Boolean);
      const fullSummary = `**Scope of Works**\n\nWe are pleased to provide our quotation for the following works:\n\n${summaryParts.map((s) => `- ${s}`).join("\n")}\n\nAll works will be carried out in accordance with __BS 5839-1:2017+A2:2019__ and relevant British Standards. A completion certificate will be issued upon successful commissioning and testing.`;
