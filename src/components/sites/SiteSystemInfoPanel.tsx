  ...
  return (
    <div className="space-y-6">
      ...
      <section className="space-y-3">
        ...
        {/* Per-type device counts (manual call points / sounders / detectors).
            Optional — engineers fill in as captured during commissioning;
            templates can render a system-particulars table from them. */}
        ...
        {/* ARC details — only meaningful when arc_connected = true. Render
            the inputs unconditionally so the engineer can still capture
            them before flipping ARC connected, but signal intent with
            the muted label. */}
        {info.arc_connected !== false && (
      ...
      {/* Voice alarm (PAVA) — gated by has_pava so the section stays out
          of the way for the majority of sites that don't have one. */}
      <section className="space-y-3 rounded-lg border border-dashed bg-muted/20 p-3">
        <div className="flex items-center justify-between gap-3">
          <div>
            <h4 className="text-sm font-semibold">Voice alarm (PAVA)</h4>
            <p className="text-xs text-muted-foreground">
              Toggle on to capture BS EN 54-16 / 54-24 system details.
            </p>
          </div>
          <Switch
            checked={!!info.has_pava}
            onCheckedChange={(v) => patch({ has_pava: v })}
            aria-label="Site has a PAVA system"
          />
        </div>
        ...
        {info.has_pava && (
          <div className="space-y-3 pt-1">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              ...
              <Field label="Software version">
                <Input
                  ...
                  onChange={(e) => patch({ pava_software_version: e.target.value })}
                />
              </Field>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              <Field label="Fire alarm interface method">
                <Input
                  ...
                  onChange={(e) => patch({ pava_fa_interface_method: e.target.value })}
                  placeholder="e.g. Volt-free contact, hardwired"
                />
              </Field>
              <Field label="Network topology">
                <Input
                  value={info.pava_network_topology ?? ""}
                  onChange={(e) => patch({ pava_network_topology: e.target.value })}
                  placeholder="e.g. Loop, A-B"
                />
              </Field>
            </div>
            ...
            <div className="flex items-center gap-2 pt-1">
              <Switch
                checked={!!info.pava_has_backup_amplifier}
                onCheckedChange={(v) => patch({ pava_has_backup_amplifier: v })}
                aria-label="Backup amplifier fitted"
              />
              <Label className="text-sm">Backup amplifier fitted</Label>
            </div>
          </div>
        )}
      </section>
