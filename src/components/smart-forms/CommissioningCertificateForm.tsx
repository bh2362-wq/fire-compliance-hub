...
export default function CommissioningCertificateForm({ open, onOpenChange, visitId, siteId, customerId, prefill, existing, onSaved }: Props) {
  const { user } = useAuth();
  ...
  useEffect(() => {
    ...
    if (existing) {
      ...
      setPayload({ ...emptyPayload(), ...(existing.payload as CommissioningPayload) });
    } else {
    ...
    <DocDialogShell open={open} onOpenChange={onOpenChange}>
      ...
      <DocBody>
        ...
        <PhotoAnalysisBlock
          ...
          context={[payload.premises_name, payload.panel_manufacturer, "BS5839 commissioning"].filter(Boolean).join(", ")}
