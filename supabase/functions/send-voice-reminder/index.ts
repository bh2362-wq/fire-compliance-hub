 ...
 const handler = async (req: Request): Promise<Response> => {
   ...
   try {
     ...
     const { reminder_id, to, message, invoice_number, amount_due, company_name }: VoiceReminderRequest = await req.json();
 ...
 <Response>
   ...
   <Say voice="alice" language="en-GB">Goodbye.</Say>
     ...
     if (!twilioResponse.ok) {
       ...
       if (reminder_id) {
         await supabase
           ...
           .update({
             status: "failed",
             error_message: twilioResult.message || "Voice call failed",
           })
           .eq("id", reminder_id);
       }
       
       throw new Error(twilioResult.message || "Failed to make voice call");
     }
 
     console.log("Voice call initiated:", twilioResult.sid);
     ...
     console.error("Error in send-voice-reminder:", error);
     return new Response(
